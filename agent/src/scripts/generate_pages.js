    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_d) {
            switch (_d.label) {
                ...
                case 1:
                    _a = _d.sent(), accRes = _a[0], pplRes = _a[1];
                ...
                case 2:
                    ...
                    for (_i = 0, accData_1 = accData; _i < accData_1.length; _i++) {
                        ...
                        console.log("\u2705 Account: ".concat(safeName, ".md"));
                    }
                    _d.label = 3;
                case 3:
                    if (!pplRes.ok) return [3 /*break*/, 10];
                    return [4 /*yield*/, pplRes.json()];
                case 4:
                    pplData = _d.sent();
                    _b = 0, pplData_1 = pplData;
                    _d.label = 5;
                case 5:
                    if (!(_b < pplData_1.length)) return [3 /*break*/, 10];
                    p = pplData_1[_b];
                    safeName = p.name.replace(/\//g, '-');
                    return [4 /*yield*/, fetch("".concat(SUPABASE_URL, "/rest/v1/transactions?person_id=eq.").concat(p.id, "&select=occurred_at&order=occurred_at.desc"), { headers: headers })];
                case 6:
                    txnRes = _d.sent();
                    years = [];
                    if (!txnRes.ok) return [3 /*break*/, 8];
                    return [4 /*yield*/, txnRes.json()];
                case 7:
                    txns = _d.sent();
                    yearSet = new Set(txns.map(function (t) { return new Date(t.occurred_at).getFullYear(); }));
                    years = Array.from(yearSet).sort().reverse();
                    _d.label = 8;
                case 8:
                    personDir = path_1.default.join(peopleDir, safeName);
                    if (!fs_1.default.existsSync(personDir))
                        fs_1.default.mkdirSync(personDir, { recursive: true });
                    indexPath = path_1.default.join(peopleDir, "".concat(safeName, ".md"));
                    fs_1.default.writeFileSync(indexPath, peopleIndexPage(p, years), 'utf8');
                    console.log("\u2705 People index: ".concat(safeName, ".md (").concat(years.length, " n\u0103m: ").concat(years.join(', '), ")"));
                    // Write year pages
                    for (_c = 0, years_1 = years; _c < years_1.length; _c++) {
                        year = years_1[_c];
                        yearPath = path_1.default.join(personDir, "".concat(year, ".md"));
                        fs_1.default.writeFileSync(yearPath, peopleYearPage(p, year), 'utf8');
                        console.log("   \uD83D\uDCC5 Year page: ".concat(safeName, "/").concat(year, ".md"));
                    }
                    _d.label = 9;
                case 9:
                    _b++;
